...
Project `.fiber.json` accepts only repo-safe defaults (`sandbox`, `max_agent_steps`, `max_tool_result_bytes`, `context`); every profile-owned key is dropped from it before its value is parsed. Environment variables override profile workspace settings, which override profile global settings, then project defaults, then built-in defaults. `FIBER_STATE_DIR` is environment-only: the state root must resolve before `settings.json` can be read, so it sits above this layering rather than in it. `src/core/config/config_runtime.zig` implements the layering.
