...
//! ignored so the integration cannot block or terminate a fiber session.
...
pub const Client = struct {
    ...
    /// Report fiber's session id so herdr can associate the pane with a resumable
    ...
    /// Name the pane/agent so herdr lists fiber even while idle. Best-effort;
    ...
    /// Remove fiber from the pane on exit: drop the reported agent and clear the
    /// label/name. Without this the pane keeps showing "fiber" after fiber is gone.
...
test "pane.rename labels the pane" {
    var out: std.Io.Writer.Allocating = .init(std.testing.allocator);
    defer out.deinit();
    try writeRename(&out.writer, 4, "pane.rename", "pane_id", "w1:p1", "label", "fiber");
    try std.testing.expectEqualStrings(
        "{\"id\":\"4\",\"method\":\"pane.rename\",\"params\":{\"pane_id\":\"w1:p1\",\"label\":\"fiber\"}}\n",
        out.written(),
    );
}
...
test "agent.rename names the agent" {
    var out: std.Io.Writer.Allocating = .init(std.testing.allocator);
    defer out.deinit();
    try writeRename(&out.writer, 5, "agent.rename", "target", "w1:p1", "name", "fiber");
    try std.testing.expectEqualStrings(
        "{\"id\":\"5\",\"method\":\"agent.rename\",\"params\":{\"target\":\"w1:p1\",\"name\":\"fiber\"}}\n",
        out.written(),
    );
}
...
test "clear_agent_authority removes fiber from the pane" {
    var out: std.Io.Writer.Allocating = .init(std.testing.allocator);
    defer out.deinit();
