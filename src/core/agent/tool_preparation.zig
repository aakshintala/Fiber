...
/// Bounded canonical tool targets retained from session history and recovery
/// state. Reads typed history only; assistant prose is never a target source.
/// Attribution: reimplements upstream fx 2131fb3 under Fiber contracts.
pub fn retainedContextTargets(
    ...
    try checkCancellation(collector.cancel_flag);
