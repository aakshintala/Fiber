...
pub const SignInRuntime = struct {
    ...
    fn workerMain(self: *Self, alloc: Allocator) void {
        while (true) {
            self.pulse(alloc);
            switch (self.transitionOrActive()) {
                .active => {},
                .finished => return,
            }
            io_mod.sleep(poll_wait_slice_ms * std.time.ns_per_ms);
        }
    }

    const WorkerStep = enum { active, finished };

    fn transitionOrActive(self: *Self) WorkerStep {
        self.mutex.lockUncancelable(io_mod.getIo());
        defer self.mutex.unlock(io_mod.getIo());
        return if (self.state == .polling) .active else .finished;
    }
...
const SignInTestState = struct {
    ...
    fn complete(
        ...
        const completion: SignInCompletion = .{ .chatgpt = .{
