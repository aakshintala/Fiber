...
**2026-09-03 direct-read findings (both stop conditions checked, neither fires — proceeding):**
- Relaunch handoff: confirmed dormant-not-regressed. `auto_upgrade.zig`'s background loop is the only production path that ever sets `AutoUpgrade.state = .ready`; removing its trigger leaves the ctrl+g / relaunch-request / `app_upgrade_runtime.zig` machinery fully intact but permanently unreached until Phase 6 wires a new upgrade producer. Matches the retained "inert but observable-nothing" seam pattern already used for the WASI-target and secret-store slices — keep `getState`, `statusLabel`, `requestRelaunch`/`takeRelaunchRequest`, `RelaunchRequest`, `render_dirty`; delete `runLoop`, `runOnce`, `downloadAndInstall`, `InstallError`, `sleepInterruptible`, `start()`, `stop()` (and its unconditional call site in `main.zig`'s `deinitImpl`), `thread`/`should_stop` fields, the three sleep/interval constants — all orphaned once nothing spawns the thread. Delete `App.startAutoUpgrade()` from `main.zig` outright (not neutered in place) so the `@hasDecl` gate in `app_entry_runtime.zig` naturally skips it; leave the `TestApp.startAutoUpgrade` test fixture alone, it tests the generic entry-runtime wiring mechanism, not this feature. Delete `FX_AUTO_UPGRADE`/`auto_upgrade_enabled`/`shouldEnableForCurrentExecutable`/`host_profile.auto_upgrade` gate at `main.zig:581-590` together with the method; `app_upgrade_runtime.zig`'s `auto_upgrade_enabled` field/reads (lines ~147-148, 235, 411) and `app_bootstrap_runtime.zig`'s (lines ~289, 561) are a *different*, still-consumed settings-derived flag (`settings.auto_upgrade`) gating whether relaunch-on-ready fires at all — leave that one alone, it's orthogonal to the CDN fetch being removed.
- Channel consumers outside distribution: `update_channel` **is** the same `update_target.Channel` type end to end — settings store, config runtime, `StartupState`/`StartupStatus`, `AutoUpgrade.configure_channel`/`selected_channel`, CLI `--channel`, and `fx status`'s `update_channel` field are all one connected preference-selection feature, not a separate concern from "Channel.dev and its comparison paths." Confirmed the reverse risk instead: `config_runtime.zig`'s settings JSON parser (~line 1407) calls `Channel.parse()` on the stored value and returns `error.InvalidUpdateChannelValue` on failure — leaving that key with a shrunk enum would hard-fail startup for any real `~/.fx/settings.json` still holding `"update_channel":"dev"`. Resolution: delete the settings key entirely (field, JSON read/write/merge, `UserPreferenceField`/migration-snapshot entry, `isProfileOnlySettingKey`-style list entry), mirroring the `.provider`/`.cost` treatment in Slice 12 (`8fd4c9b4`) exactly — an old `"update_channel"` value in a settings file then just becomes silently-ignored stale JSON, not a validation error. Same for the CLI `upgrade --channel` flag (`UpgradeOptions.channel`, `parseUpgradeArgs`, the `commands.zig` usage string) and `StartupState.update_channel`/`StartupStatus.update_channel` — delete the fields; the two `fx status` JSON call sites (`cli_surface.zig:1366`, and the two prebuilt fixtures around 782/1208) hardcode `.update_channel = update_target.Channel.stable.label()` so the output *shape* (`"update_channel":"stable"`) is unchanged, avoiding a JSON contract break for anyone parsing `fx status`. `AutoUpgrade.configure_channel`/`selected_channel` are deleted (nothing sets them anymore); its `channel()` method is deleted too — `session_commands.zig`'s `update_channel_label` already falls back to the literal `"stable"` via its existing `@hasDecl` capability check, zero-touch there.
- `cdn_base`/`resolveCdnBase()` in `upgrade_helpers.zig`: the file text above ("no compiled-in default, Phase 6 supplies GitHub Releases") is the correct scope, not a misreading — `resolveCdnBase()` becomes `?[]const u8` (`FX_E2E_UPGRADE_BASE_URL` loopback override still returns a value; otherwise `null`, the compiled literal `"https://releases.fx.sh"` is deleted). Both callers (`upgrade_runtime.zig`'s `upgradeWorkerInner`, and previously `auto_upgrade.zig`'s now-deleted `runOnce`) gain an `orelse { result.err = .fetch_failed; return; }` guard using the existing `.fetch_failed` `WorkerResult` error variant — no new error type needed. `fetchLatestVersion`/`fetchTextBounded`/`downloadFileStreaming*`/`verifyChecksum`/`platform` are untouched; only `fetchTarget`'s `.dev` switch arm goes (single `.stable` arm retained, `channel: Channel` parameter kept — matches the `ProviderId` single-variant-seam precedent, not collapsed away).
- `update_target.zig`: delete `Channel.dev`, `Target.Dev`, `Target.parseDevManifest`, and every `.dev` switch arm (`deinit`, `version`, `revision`, `artifactRef`, `writeDisplayLabel`). `shouldInstall`'s `if (self.channel() != current.channel) return true` is now a provably-always-false comparison (one variant both sides) — simplify to just the stable version-compare branch. Delete the two dev-only tests; trim the two mixed tests (channel-parsing, freshness) to their stable-only assertions.
- `build.zig`/`src/ui/render.zig`/`src/main.zig`: `build.zig`'s `UpdateChannel` enum and `-Dupdate-channel` option are deleted; `build_options.addOption([]const u8, "update_channel", ...)` becomes a hardcoded `"stable"` literal so `main.zig:101`'s and `render.zig:168`'s `compiled_update_channel`/`build_channel` derivations keep compiling unchanged (simplify both to a literal `.stable` if that reads cleaner — no `@compileError` path can fire anymore either way). **Leave `.github/workflows/dev-release.yml` untouched** — Slice 20 already owns deleting it outright; between this slice landing and Slice 20 landing, that workflow's `-Dupdate-channel=dev` build step will fail (non-blocking `workflow_run`, not part of the four local gates), a narrow self-resolving gap, not a stop condition.
- `tests/e2e/cli.test.ts` channel assertions: left untouched, consistent with every prior slice this session — e2e is deferred/ungated project-wide.
- Net: this widens the slice's touched-file count materially versus the original one-paragraph estimate (adds `build.zig`, `src/ui/render.zig`, `src/main.zig`'s `App.startAutoUpgrade`/`FX_AUTO_UPGRADE` block, `session_commands.zig` gets a free ride via its existing fallback) but every addition is a direct, compile-required consequence of deleting `Channel.dev`, not scope creep.
...
**2026-09-03 direct-read findings (investigated while Slice 19's write was in flight, not yet executed):**
- `ci.yml`'s `cross-target` matrix `macos-x86_64` entry runs on `runs-on: ubuntu-latest` (cross-compiling to that target, no Intel hardware involved) — still an Intel-target removal per the inventory's own instruction, not a false-positive guard; delete that matrix entry same as the others.
- `.github/workflows/publish-libfx.yml` no longer exists in the repo (already deleted in an earlier slice), but `scripts/tests/test_macos_signing.py` still has a `WORKFLOW_PATH`-style constant pointing at it plus `build-macos-x86_64`/`runs-on: macos-15-intel`/`needs: [...build-macos-x86_64...]` assertions (~lines 18, 478-502) — all stale, delete.
- `scripts/tests/test_binary_size.py:252` has one `("macos-x86_64", "x86_64-macos", "macos-15-intel")` matrix-fixture tuple to delete.
- Verified via direct grep what's actually still live vs. dead in the code, so the doc strip doesn't undershoot or overshoot:
  - `ProviderId` (`src/core/config/model_provider.zig`) is already down to `.codex` only — Gateway and Grok login are both **fully gone**, not just Grok. README's Gateway sign-in section (`fx login` bare, lines ~30-34), the Grok section (`fx login grok`/`fx login codex` comparison framing, lines ~43-48), the `/setup`/multi-provider-switching paragraph (line ~50), the Grok-route paragraph (line ~54), and the "AI Gateway API key" / `fx setup` section (lines ~56-60) are **all** dead — collapse README's whole "Run fx" section down to just the `fx login codex` flow, not a narrower Grok-only edit.
  - `/setup` and `fx setup` (CLI or slash form): no hits in `command_specs.zig`/`src/builtins/commands.zig` — confirmed gone.
  - `/feedback`: confirmed gone from `command_specs.zig`/`command_router.zig` (only test assertions that it's `.unknown` remain, which is correct, leave those). README line ~92 ("Run `/feedback` to open the feedback form...") is stale, strip it.
  - `CONTRIBUTING.md:27` (Vercel OAuth via `fx login`, `fx setup` Keychain API keys, `AI_GATEWAY_API_KEY`, `VERCEL_OIDC_TOKEN`) is the same dead Gateway-auth surface as the README — strip. `CONTRIBUTING.md:421` describes the exact `releases.fx.sh`/Vercel-Blob CDN contract Slice 19 removes — rewrite once Slice 19's code change is in (this file's `421` line number will drift once Slice 19 lands, re-locate by content, not line number). `CONTRIBUTING.md:332`'s "permission feedback" is unrelated prose (permission-review terminology, not the `/feedback` command) — a false-positive for any naive `feedback` search-and-strip, leave it.
  - Remaining `grok`/`Grok` hits in `src/` (`model_menu_presentation.zig`, `model_cache_runtime.zig`, `app_input_runtime.zig`, `model_catalog.zig`, `responses_protocol.zig`) are all generic example model-id strings (`xai/grok-4`, `xai/grok-flash`, etc.) flowing through the Gateway model-catalog display/test fixtures — unrelated to the removed Grok *provider*/login flow, not in scope, do not touch.
  - `ai-gateway.vercel.sh` also appears in `src/core/cli/cli_ask.zig`, `src/core/session/usage_recovery.zig`, `src/core/session/session_log.zig` (5 hits, test/fixture data, likely historical-session-log-compatibility fixtures) — outside Slice 20's stated file list (workflows + docs + the two python test scripts); the post-deletion search list should be read as scoped to those files, not a signal to sweep `src/`. Left alone; flagging so a literal `rg ai-gateway.vercel.sh` sweep doesn't get treated as a checklist against files this slice never touches.
  - README's clone URL (line ~143, `git clone https://github.com/vercel-labs/fx.git`) should become Fiber's own repo URL; the Apache-2.0 attribution block (lines ~155-158, "Fiber is a fork of fx... Copyright 2025 Vercel, Inc... Attribution details are in NOTICE") is the one `vercel-labs`/Vercel mention in this file that must survive verbatim — do not let a blind `vercel-labs`/`Vercel` strip touch it.
- Not yet done: full line-by-line strip of README.md/CONTRIBUTING.md, the workflow edits, or the two python test-script edits. This is a mapping pass only, performed opportunistically while Slice 19's cursor-delegate job was still running; execute Slice 20 as its own dispatch once Slice 19 is committed (its "Dependencies: Slice 19" line is about the updater docs specifically, but touching `CONTRIBUTING.md:421`'s CDN description before Slice 19's code lands would describe an already-wrong state, so wait for the full commit regardless).

**2026-09-03, Slice 19 landed (`4b51f28a`); executing Slice 20. Parking one sub-item, proceeding with the rest:**
- **Parked, needs owner input, not attempted:** `prepare-release.yml`'s "remove the Vercel AI Gateway changelog generation (around lines 133-140)" instruction. Direct read shows lines 133-140 are just the `curl` call inside a much larger step (~line 90-200): a jq-built request payload feeds it, and an SSE-response parse plus a Python leak-check/format-policy block (including the `vercel-labs` regex this same bullet also names) consumes its output into `/tmp/changelog-body.md`, which the next step ("Update version and changelog") then splices into `CHANGELOG.md`. Deleting only the 8-line curl call leaves the payload-building and response-parsing code dangling against nothing; deleting the whole step leaves "Update version and changelog" with no changelog body and no stated replacement. What replaces AI-generated release notes (manual changelog entry required before merge? drop the changelog step and just bump the version?) is a real decision, not a mechanical strip, and reads as Phase 6 "final documentation and release preparation" territory rather than this slice's. Left `prepare-release.yml` entirely untouched, including the `vercel-labs` leak-check regex (coupled to the same step) and a pre-existing, already-dead `sed -i "s|bash -s v${CURRENT}|bash -s v${NEW}|g" README.md` line in the same "Update version and changelog" step (matches no string in the current README today, predates this session, unrelated to this slice's README edits either way).
