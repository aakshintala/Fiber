...
- Paths and symbols: `src/core/app/app_entry_runtime.zig` (`runInteractiveCooperative`, cooperative parameter); `src/core/auth/login_flow.zig` (`startPreparedCooperative`); `src/main.zig` (`cooperativeTransportPulse`, `processNextCooperativePrompt`); `src/core/app/app_callbacks.zig`; `src/core/agent/runtime/deps.zig`; `src/core/agent/runtime/orchestrator.zig`; `src/core/agent/stream_provider.zig` (`cooperative_pulse`); `src/core/hosts/runtime_profile.zig` (`cooperative_agent`).
- Retained invariants: native threaded prompt execution, sign-in polling, cancellation, and subagents.
- Exact post-deletion searches: `runInteractiveCooperative`, `startPreparedCooperative`, `cooperativeTransportPulse`, `processNextCooperativePrompt`, `CooperativePulse`, `cooperative_pulse`, `cooperative_agent`.
