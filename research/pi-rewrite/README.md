...
- **Tier 2, per medium.** #7 found that "a drawing surface is always a surface *for one medium*": pi's `ctx.ui.custom()` cannot be reached from a React GUI without embedding a terminal. So above the portable tier, code is written per medium. That is exactly what a facet is. Fiber named this tier as a future outside v0.0.1, shaped after #11. It is recorded in the map's out-of-scope section, with this section as its reference.
