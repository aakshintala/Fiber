# Writing a Fiber extension — Luau

The runtime is Luau (Roblox's Lua dialect) with a stripped standard library and `sandbox` mode on: globals are read-only. There is no `io`, `os`, or `debug`. JSON is provided by the host as `json.decode` / `json.encode`. Luau supports optional type annotations; you may use them.
...
```
json.decode(str)  -- host-provided; returns a table
```
