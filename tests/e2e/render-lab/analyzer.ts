...
export function analyzeRun(manifest: RenderLabManifest) {
  ...
  for (const frame of frames) {
    ...
    if (countLogoBlocks(frame.grid, logoRows) > 1) {
      push(failures, "single-active-logo", frame, "more than one active fiber logo block is visible");
    }
    ...
    if (expectsChrome && footers.length === 0 && !viewerFooterPresent) {
      push(failures, "footer-missing", frame, "fiber-owned frame has no complete footer block");
    }
    ...
    if (expectsChrome && input_rows.length === 0 && !viewerFooterPresent) {
      push(failures, "input-missing", frame, "fiber-owned frame has no footer input row");
    }
    ...
    if (fxBand) {
      for (const marker of manifest.markers.shell) {
        ...
        if (badRows.length > 0) {
          push(
            failures,
            "shell-marker-in-fiber-band",
            frame,
            `shell marker ${marker} appears inside the fiber-owned viewport band`,
          );
        }
      }
    }
...
function expectsFxChrome(
  ...
  if (
    ...
    frame.event.includes("fiber-quit-requested") ||
