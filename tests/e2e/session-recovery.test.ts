...
describe("session recovery", () => {
  for (const boundary of [
    ...
    test(
      ...
      async () => {
        const root = mkdtempSync(join(tmpdir(), "fiber-session-pre-authority-"));
        try {
  ...
  for (const boundary of [
    ...
    test(
      ...
      async () => {
        const root = mkdtempSync(join(tmpdir(), "fiber-session-post-authority-"));
        try {
          const home = join(root, "home");
          const workspace = join(root, "workspace");
          const ready = join(root, "boundary.ready");
  ...
  test("doctor removes only a validated noncurrent watermark", async () => {
    const root = mkdtempSync(join(tmpdir(), "fiber-session-doctor-cleanup-"));
    try {
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      mkdirSync(home);
  ...
  test("session recover copies a corrupt-watermark session without changing its source", async () => {
    const root = mkdtempSync(join(tmpdir(), "fiber-session-copy-recovery-"));
    try {
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      mkdirSync(home);
      ...
      expect(doctor.stdout).toContain(`fiber session recover ${sessionId}`);
      ...
      expect(JSON.parse(sourceDetail.stdout)).toEqual(
        expect.objectContaining({
          ...
          error: `session ${sessionId} is corrupt; run \`fiber session recover ${sessionId}\``,
        }),
      );
    } finally {
      rmSync(root, { recursive: true, force: true });
  ...
  test("cross-workspace recovery preserves both resume-last pointers", async () => {
    const root = mkdtempSync(join(tmpdir(), "fiber-session-cross-workspace-recovery-"));
    try {
      const home = join(root, "home");
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "fiber-session-recovery-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        const ready = join(root, "boundary.ready");
  ...
  for (const boundary of [
    ...
    test(
      ...
      async () => {
        const root = mkdtempSync(join(tmpdir(), "fiber-session-commit-"));
        try {
          const home = join(root, "home");
          const workspace = join(root, "workspace");
          const ready = join(root, "boundary.ready");
