...
const wireLogPath = process.env.FIBER_MCP_WIRE_LOG;
const pidPath = process.env.FIBER_MCP_PID_PATH;
const resultText = process.env.FIBER_MCP_RESULT_TEXT ?? "MODERN_MCP_TOOL_RESULT";
const mode = process.env.FIBER_MCP_MODE ?? "normal";
const crashMarkerPath = process.env.FIBER_MCP_CRASH_MARKER;
const recoveryFailureMarkerPath = crashMarkerPath
  ? `${crashMarkerPath}.recovery-failed`
  : undefined;
const recoveryReadyPath = process.env.FIBER_MCP_RECOVERY_READY_PATH;
const recoveryReleasePath = process.env.FIBER_MCP_RECOVERY_RELEASE_PATH;
const initialToolName = process.env.FIBER_MCP_INITIAL_TOOL_NAME ?? "echo";
const recoveredToolName = process.env.FIBER_MCP_RECOVERED_TOOL_NAME ?? initialToolName;
const expectedElicitation = process.env.FIBER_MCP_EXPECT_ELICITATION ?? "none";
const environmentCapturePath = process.env.FIBER_MCP_ENV_CAPTURE;
const resourcesSubscribe = process.env.FIBER_MCP_RESOURCES_SUBSCRIBE !== "0";
const resourceTtlMs = process.env.FIBER_MCP_RESOURCE_TTL_MS === undefined
  ? null
  : Number(process.env.FIBER_MCP_RESOURCE_TTL_MS);
const catalogDelayMs = Math.max(
  0,
  Number(process.env.FIBER_MCP_CATALOG_DELAY_MS ?? "0") || 0,
);
const elicitationUrl = process.env.FIBER_MCP_ELICITATION_URL ?? "https://example.test/connect";
...
if (environmentCapturePath) {
  writeFileSync(environmentCapturePath, JSON.stringify({
    configured: process.env.FIBER_MCP_ENV_SENTINEL,
    inherited: process.env.FIBER_MCP_INHERITED_SENTINEL,
