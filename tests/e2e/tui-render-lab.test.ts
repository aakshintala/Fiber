...
test.skipIf(!existsSync(FIBER_BIN))("buffer-system frame benchmark writes timing artifact", () => {
  outDir = mkdtempSync(join(tmpdir(), "fiber-frame-bench-test-"));
...
test.skipIf(!existsSync(FIBER_BIN))("buffer-system p95 gate rejects undersampled runs", () => {
  outDir = mkdtempSync(join(tmpdir(), "fiber-frame-bench-samples-test-"));
...
test("native render-lab scenarios require explicit opt-in", () => {
  outDir = mkdtempSync(join(tmpdir(), "fiber-render-lab-gate-test-"));
...
test("render-lab analyzer flags excess gap before transient activity", () => {
  outDir = mkdtempSync(join(tmpdir(), "fiber-render-lab-analyzer-test-"));
...
test("render-lab analyzer recognizes the real fiber prompt glyph", () => {
  outDir = mkdtempSync(join(tmpdir(), "fiber-render-lab-analyzer-test-"));
  const manifest = writeAnalyzerFixture(outDir, [
...
test("render-lab analyzer rejects damaged multiline footer with real hint and cursor", () => {
  ...
  const manifest = writeAnalyzerFixture(outDir, [
    ...
    "  transcript row two",
    "────────────────",
    "run fiber login · opus 4.7 · default",
  ], { row: 3, col: 8 });
...
describe.skipIf(SKIP)("tui: render lab", () => {
  ...
  test(
    ...
    () => {
      outDir = mkdtempSync(join(tmpdir(), "fiber-render-lab-test-"));
      const env = { ...process.env };
  ...
  test(
    ...
    () => {
      ...
      expect(artifacts.gatewayRequests).toEqual(["GET /models", "POST /responses"]);
      expect(artifacts.stderr).toBe("");
    },
    TIMEOUT,
  );
...
function runScenarioArtifacts(scenario: string): {
  ...
  outDir = mkdtempSync(join(tmpdir(), "fiber-render-lab-overflow-test-"));
  const env = { ...process.env };
  const output = execFileSync(
    "bun",
