...
describe("agent quality A/B harness helpers", () => {
  ...
  test("rejects bare fx and relative binary paths", () => {
    ...
    expect(() => requireAbsoluteExecutableBinary("./zig-out/bin/fiber", "candidate")).toThrow(/absolute path/);
  });

  test("redacts credential-looking values", () => {
    expect(redactSensitiveValue("AI_GATEWAY_API_KEY", "secret-value")).toBe("[redacted]");
    expect(redactSensitiveValue("FIBER_MODEL", "provider/test-model")).toBe("provider/test-model");
  });
...
const hasLiveAbConfig = Boolean(
  process.env.FIBER_AB_BASELINE_BIN &&
    process.env.FIBER_AB_CANDIDATE_BIN &&
    process.env.FIBER_AB_MODEL &&
